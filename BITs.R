...
## Calcualte Densities for metagene plot:
metaDensity = function(feature_df, peak_matrix, feature_center, window_width = 20, window_definition = 500) {
  feature_df_sense = feature_df %>% filter(strand == '+')
  feature_df_antisense = feature_df %>% filter(strand == '-')
  
  peaks_df_sense = peak_matrix %>% filter(strand == '+')
  peaks_df_antisense = peak_matrix %>% filter(strand == '-')
  
  feature_center_sense = feature_center
  if (feature_center_sense == 'start') {
    feature_center_antisense = 'end'
  } else if (feature_center_sense == 'end') {
