...
# peaksMatrix_PATH = 'L:/My Drive/CWRU/PhD/Luna Lab/1. coCLIP/peakMatrix/'  ## Use this for windows machine
...
peakMatrix = read_delim(paste0(peakPATH, peakFILE), show_col_types = FALSE)
...
## Filter by BC
snoRNA_Enrichment1 = snoRNA_Enrichment %>% filter(Nuc_F_M_BC >= 3, Cyto_F_M_BC >= 3, NLS_E_M_BC >= 4, NES_E_M_BC >= 4)

xlims = c(0.6, 1.2)
ylims = c(0.6, 1.2)

snoRNA_F_M_NvC = log(snoRNA_Enrichment1$Nuc_F_M, 2) / log(snoRNA_Enrichment1$Cyto_F_M, 2)
snoRNA_E_M_NvC = log(snoRNA_Enrichment1$NLS_E_M, 2) / log(snoRNA_Enrichment1$NES_E_M, 2)
...
snoRNA_F_S_NvC = log(snoRNA_Enrichment1$Nuc_F_S, 2) / log(snoRNA_Enrichment1$Cyto_F_S, 2)
snoRNA_E_S_NvC = log(snoRNA_Enrichment1$NLS_E_S, 2) / log(snoRNA_Enrichment1$NES_E_S, 2)
