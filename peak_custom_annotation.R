...
# write.table(listAttributes(mart.hs), "Attributes_Hs.txt",  quote=F, sep="\t")
ensembl.tx = getBM(attributes=c("ensembl_gene_id", "external_gene_name", "hgnc_symbol"), mart = mart.hs) 
#write.table(ensembl.tx, "Ensembl_gene_id_2_gene_name.txt", quote=F, sep="\t")


## Load peak file:
x = read.delim("/Users/soonyi/Desktop/Genomics/CoCLIP/Analysis/Combined_peakCoverage_groomed.txt", header = TRUE, sep = "\t")
x = x %>% mutate(chr = ifelse(chr == "chrM", "chrMT", chr))
names(x)[names(x) == 'chr'] = 'chrom'
x = data.frame(x, row.names = 4)

## Create GRanges object for CLIP peak regions
gr.CombinedCoCLIP = GRanges(seqnames=x$chrom, ranges=IRanges(start=x$start, end=x$end, names=row.names(x)), strand=x$strand)
...
x$fiveUTRs = fiveUTRs.CombinedCoCLIP[,1]
x$threeUTRs = threeUTRs.CombinedCoCLIP[,1]
x$CDS = CDS.CombinedCoCLIP[,1]
x$DNS10K = DNS10K.CombinedCoCLIP[,1]
x$introns = introns.CombinedCoCLIP[,1]
x$tx_ext = tx_ext.CombinedCoCLIP[,1]
x$gene = genes.bed[genes.CombinedCoCLIP[,1], "name"]
...
x$tRNA = tRNA.bed[tRNA.CombinedCoCLIP[,1], "name"]
x$tRNA1 = tRNA.CombinedCoCLIP[,1]
x$LINE = LINE.bed[LINE.CombinedCoCLIP[,1], "name"]
x$LINE1 = LINE.CombinedCoCLIP[,1]
x$LTR = LTR.bed[LTR.CombinedCoCLIP[,1], "name"]
x$LTR1 = LTR.CombinedCoCLIP[,1]
x$LC_SR = LC_SR.bed[LC_SR.CombinedCoCLIP[,1], "name"]
x$LC_SR1 = LC_SR.CombinedCoCLIP[,1]
x$Satellite = Satellite.bed[Satellite.CombinedCoCLIP[,1], "name"]
x$Satellite1 = Satellite.CombinedCoCLIP[,1]
x$SINE = SINE.bed[SINE.CombinedCoCLIP[,1], "name"]
x$SINE1 = SINE.CombinedCoCLIP[,1]
x$miR = miR[miR.CombinedCoCLIP[,1], "gene_name"]
x$miR1 = miR.CombinedCoCLIP[,1]
x$lncRNA = lncRNA[lncRNA.CombinedCoCLIP[,1], "gene_name"]
x$lncRNA1 = lncRNA.CombinedCoCLIP[,1]
x$rRNA = rRNA[rRNA.CombinedCoCLIP[,1], "gene_name"]
x$rRNA1 = rRNA.CombinedCoCLIP[,1]
x$snoRNA = snoRNA[snoRNA.CombinedCoCLIP[,1], "gene_name"]
x$snoRNA1 = snoRNA.CombinedCoCLIP[,1]
x$scaRNA = scaRNA[scaRNA.CombinedCoCLIP[,1], "gene_name"]
x$scaRNA1 = scaRNA.CombinedCoCLIP[,1]
x$snRNA = snRNA[snRNA.CombinedCoCLIP[,1], "gene_name"]
x$snRNA1 = snRNA.CombinedCoCLIP[,1]
x$miscRNA = miscRNA[miscRNA.CombinedCoCLIP[,1], "gene_name"]
x$miscRNA1 = miscRNA.CombinedCoCLIP[,1]
# x$anti = anti[anti.CombinedCoCLIP[,1], "gene_name"]
# x$anti1 = anti.CombinedCoCLIP[,1]
x$Prot_retained_int = Prot_retained_int[Prot_retained_int.CombinedCoCLIP[,1], "transcript_name"]
x$Prot_retained_int1 = Prot_retained_int.CombinedCoCLIP[,1]
x$nc_retained_int = nc_retained_int[nc_retained_int.CombinedCoCLIP[,1], "transcript_name"]
x$nc_retained_int1 = nc_retained_int.CombinedCoCLIP[,1]
...
x$fiveUTRs = ifelse(is.na(x$fiveUTRs), NA, "5'UTR")
x$threeUTRs = ifelse(is.na(x$threeUTRs), NA, "3'UTR")
x$CDS = ifelse(is.na(x$CDS), NA, "CDS")
x$DNS10K = ifelse(is.na(x$DNS10K), NA, "downstream 10K")
x$introns = ifelse(is.na(x$introns), NA, "intron")
x$intergenic = ifelse(is.na(x$tx_ext), "deep intergenic", NA)
x$tRNA1 = ifelse(is.na(x$tRNA1), NA, "tRNA")
x$LINE1 = ifelse(is.na(x$LINE1), NA, "TE")
x$LTR1 = ifelse(is.na(x$LTR1), NA, "Other")
x$LC_SR1 = ifelse(is.na(x$LC_SR1), NA, "Other")
x$Satellite1 = ifelse(is.na(x$Satellite1), NA, "Other")
x$SINE1 = ifelse(is.na(x$SINE1), NA, "TE")
x$miR1 = ifelse(is.na(x$miR1), NA, "miRNA")
x$lncRNA1 = ifelse(is.na(x$lncRNA1), NA, "lncRNA")
x$rRNA1 = ifelse(is.na(x$rRNA1), NA, "rRNA")
x$snoRNA1 = ifelse(is.na(x$snoRNA1), NA, "snoRNA")
x$scaRNA1 = ifelse(is.na(x$scaRNA1 ), NA, "scaRNA")
x$snRNA1 = ifelse(is.na(x$snRNA1), NA, "snRNA")
x$miscRNA1 = ifelse(is.na(x$miscRNA1 ), NA, "miscRNA")
# x$anti1 = ifelse(is.na(x$anti1), NA, "antisense")
x$Prot_retained_int1  = ifelse(is.na(x$Prot_retained_int1), NA, "CDS_Retained_intron")
x$nc_retained_int1  = ifelse(is.na(x$nc_retained_int1), NA, "ncRNA_Retained_intron")
...
x$DNS10K = apply(x, 1, function(i) ifelse(is.na(i["DNS10K"]), NA, ifelse(is.na(i["threeUTRs"]), "downstream 10K", NA)))
...
x$annotation = apply(x, 1, function(i) paste(i[c("fiveUTRs","threeUTRs","CDS","DNS10K","introns","tRNA1","LTR1","LINE1","SINE1", "Satellite1","LC_SR1","intergenic", "miR1", "lncRNA1", "rRNA1", "snoRNA1", "scaRNA1", "snRNA1", "miscRNA1", "Prot_retained_int1", "nc_retained_int1")][which(!is.na(c(i["fiveUTRs"], i["threeUTRs"], i["CDS"], i["DNS10K"], i["introns"], i["tRNA1"], i["LTR1"], i["LINE1"], i["SINE1"],  i["Satellite1"], i["LC_SR1"], i["intergenic"], i["miR1"], i["lncRNA1"], i["rRNA1"], i["snoRNA1"], i["scaRNA1"], i["snRNA1"], i["miscRNA1"], i["Prot_retained_int1"], i["nc_retained_int1"])))], collapse="|"))
# x$annotation = apply(x, 1, function(i) paste(i[c("fiveUTRs", "threeUTRs", "CDS", "DNS10K", "introns", "intergenic", "miR1", "lncRNA1", "rRNA1", "snoRNA1", "scaRNA1", "snRNA1", "miscRNA1", "Prot_retained_int1", "nc_retained_int1")][which(!is.na(c(i["fiveUTRs"], i["threeUTRs"], i["CDS"], i["DNS10K"], i["introns"], i["intergenic"], i["miR1"], i["lncRNA1"], i["rRNA1"], i["snoRNA1"], i["scaRNA1"], i["snRNA1"], i["miscRNA1"], i["Prot_retained_int1"], i["nc_retained_int1"])))], collapse="|"))


#########################################################################################################
# Priority list
priority_list = c("5'UTR", "CDS", "3'UTR", 
...
x = x[ , -which(names(x) %in% drops)]
x$clusternames <- row.names(x)

ensembl.tx <- ensembl.tx[complete.cases(ensembl.tx[ ,3]),]

x = merge(x, ensembl.tx, by.x="gene", by.y="ensembl_gene_id", all.x=TRUE)  #all.x?

x = merge(x, df, by.x="clusternames", by.y="Row.Names", all.x=TRUE)  #all.x?
